...
# --- Critic Network Definition --- (No changes here)
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var):
        super(Critic, self).__init__()
        self.layer_1 = nn.Linear(state_dim + action_dim, n_latent_var)
...
# --- Actor Network with MDN --- (No changes here)
class ActorMDN(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var, max_action, n_gaussians=5):
        super(ActorMDN, self).__init__()
        self.action_dim = action_dim
...
class SACAgent:
    ...
    def __init__(self, state_dim, action_dim, max_action, n_latent_var, lr, gamma, tau, alpha, device):
        self.gamma = gamma
        self.tau = tau
        self.alpha = alpha
        self.device = device
        ...
        # Now we pass self.max_action directly to the Actor
        self.actor = ActorMDN(state_dim, action_dim, n_latent_var, self.max_action).to(self.device)
    ...
    def update(self, replay_buffer, batch_size):
        ...
        # --- Device Handling Step 4: Move the entire batch to the device for training ---
        state = torch.FloatTensor(state).to(self.device)
        action = torch.FloatTensor(action).to(self.device)
        reward = torch.FloatTensor(reward).unsqueeze(1).to(self.device)
        next_state = torch.FloatTensor(next_state).to(self.device)
        ...
        # --- All subsequent calculations now happen on the correct device ---

        # Calculate target Q-value
        with torch.no_grad():
            next_action, next_log_prob = self.actor.sample(next_state)
            target_q1, target_q2 = self.critic_1_target(next_state, next_action), self.critic_2_target(next_state, next_action)
            target_q = torch.min(target_q1, target_q2) - self.alpha * next_log_prob
            target_q = reward + (1 - done) * self.gamma * target_q
