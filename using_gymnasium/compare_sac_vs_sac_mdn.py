...
# --- Local Imports ---
# Use aliases to distinguish the two agent classes
from agents.sac_agent import SACAgent as SACAgentGaussian
from agents.sac_agent_mdn import SACAgent as SACAgentMDN
...
def train_and_evaluate_patient(patient_name, gpu_id, seed, hyperparameters, actor_type):
    """
    ...
    The 'actor_type' argument determines whether to use the 'gaussian' or 'mdn' agent.
    """
    ...
    print(f"Process for {patient_name} ({actor_type.upper()}) starting on device: {device}")
    ...
    # Create agent-specific directories for models and results
    AGENT_NAME = f'sac_{actor_type}_personalized_exp'
    ...
    # Conditionally instantiate the correct agent based on actor_type
    agent_class = SACAgentGaussian if actor_type == 'gaussian' else SACAgentMDN
    agent = agent_class(
    ...
    # 2. TRAINING LOOP
    # =================================================================
    print(f"--- Starting Training for Patient: {patient_name} ({actor_type.upper()}) on {device} ---")
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        if i_episode % 50 == 0:
            print(f"[{patient_name} ({actor_type.upper()}) on {device}] Episode {i_episode}/{max_episodes} | Reward: {episode_reward:.2f}")
    
    env.close()
    print(f"--- Training Finished for {patient_name} ({actor_type.upper()}) ---")
    ...
    plt.title(f'Learning Curve for {patient_name} ({actor_type.upper()})')
    plt.xlabel('Episode')
    ...
    # 3. EVALUATION LOOP
    # =================================================================
    print(f"\n--- Starting Evaluation for {patient_name} ({actor_type.upper()}) ---")
    ...
    fig.suptitle(f'Performance for {patient_name} ({actor_type.upper()})', fontsize=16, weight='bold')
...
if __name__ == '__main__':
    ...
    AGENT_TYPES_TO_TEST = ['gaussian', 'mdn']

    for agent_type in AGENT_TYPES_TO_TEST:
        print("\n" + "#"*70)
        print(f"###### STARTING EXPERIMENT FOR AGENT TYPE: {agent_type.upper()} ######")
        print("#"*70 + "\n")

        hyperparameters = {
            'max_episodes': 200, 'lr': 1e-4, 'gamma_val': 0.99, 'tau': 0.005, 'alpha': 0.2,
            'batch_size': 256, 'n_latent_var': 256, 'replay_buffer_size': 1000000,
            'max_timesteps_per_episode': 288, 'learning_starts': 1500, 'ETA': 4.0
        }
        
        results_dir = f'./results/sac_{agent_type}_personalized_exp'
        os.makedirs(results_dir, exist_ok=True)
        ...
        for i, patient_name in enumerate(adult_patients):
            gpu_id = i % num_gpus
            tasks.append((patient_name, gpu_id, 42 + i, hyperparameters, agent_type))
        ...
        print(f"\n--- ALL TRAINING PROCESSES FOR {agent_type.upper()} COMPLETE ---")
        ...
        if results:
            ...
            print("\n" + "="*70)
            print(f"--- OVERALL EVALUATION SUMMARY ({agent_type.upper()}) ---")
            print("="*70)
            ...
            try:
                ...
                plt.title(f'Glycemic Control Summary Across All Patients ({agent_type.upper()})', fontsize=18, weight='bold')
            ...
            try:
                ...
                plt.title(f'Overall Average Learning Curve ({agent_type.upper()})')
                plt.xlabel('Episode')
                plt.ylabel('Cumulative Reward')
                plt.legend()
                plt.grid(True)
            ...
            print(f"\n⚠️ No results were generated for {agent_type.upper()} to summarize.")
