...
def train_and_evaluate_patient(patient_name, gpu_id, seed, hyperparameters):
    ...
    in a closed-loop setting, now using the realistic meal generator for training.
    """
    ...
    # New agent name for this more advanced experiment
    AGENT_NAME = 'sac_realistic_scenario_exp'
    ...
    # Create a patient object to get their Body Weight (BW) for personalization
    patient_for_scenario = T1DPatient.withName(patient_name)
    # Now, use the new realistic scenario for training
    ...
    # Use a new env_id to avoid registration conflicts
    env_id = f'simglucose/sac-realistic-{clean_patient_name}-v0'
    ...
    plt.title(f'Learning Curve for {patient_name} (Realistic Scenario)')
    plt.xlabel('Episode')
    ...
    fig.suptitle(f'Performance for {patient_name} (Trained on Realistic Scenario)', fontsize=16, weight='bold')
...
if __name__ == '__main__':
    ...
    # AGENT_NAME = 'sac_realistic_scenario_exp'
    AGENT_NAME = 'sac_realistic_scenario_exp_af'
    results_dir = f'./results/{AGENT_NAME}'
    os.makedirs(results_dir, exist_ok=True)
    ...
    print("\n" + "="*50)
    print(f"Starting parallel training with REALISTIC MEALS for {len(tasks)} patients on {num_gpus} GPUs...")
    print("="*50)
